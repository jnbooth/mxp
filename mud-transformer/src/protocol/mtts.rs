...
pub const fn bitmask(config: &TransformerConfig) -> u16 {
    ...
    ANSI | mask(config.console_controls, VT100)
        | mask(!config.disable_utf8, UTF_8)
        | XTERM_COLORS
        | mask(config.mouse_tracking, MOUSE_TRACKING)
        | OSC_COLOR_PALETTE
        | mask(config.screen_reader, SCREEN_READER)
        | mask(config.proxy, PROXY)
        | TRUECOLOR
        | MNES
        | MSLP
        | mask(config.ssl, SSL)
